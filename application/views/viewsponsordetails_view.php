		...
		<h3>Student Feedback</h3>
			<?if(!empty($scholarsfeedback)) {
				foreach($scholarsfeedback as $indiv_feedback) {?>
					...
					<a href="<?=base_url('viewstudentdetails')?>/viewstudentinfo/<?=$indiv_feedback['studentid']?>"><?=$indiv_feedback['firstname'].' '.$indiv_feedback['lastname']?> (<?=$indiv_feedback['name']?>) </a> wrote about <?=$indiv_feedback['title']?> on <?=$indiv_feedback['insertedon']?>: <br/>
