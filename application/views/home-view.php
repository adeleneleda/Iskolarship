<div  style="width:90%; margin-left:4%">   
   <div class="p lead">New Scholars<span class="pull-right"><a class="btn">View all Scholars</a></span>
      <hr/>
    </div>
    <div align="center" class="row-fluid">
      <div id="myCarousel" class="carousel slide">
        <div class="carousel-inner">
          <div class="item active">
            <div style="width:85%" class="container">
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read more</a></div>
                </div>
              </div>
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read More</a></div>
                </div>
              </div>
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read More</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="item">
            <div style="width:85%;" class="container">
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read more</a></div>
                </div>
              </div>
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read More</a></div>
                </div>
              </div>
              <div class="span4">
                <div class="well profile">
                  <div class="picture"></div>
                  <div class="name">Name of Student<br/>Name of Scholarship<br/><br/><a class="btn btn-primary">Read More    </a></div>
                </div>
              </div>
            </div>
          </div>
        </div><a href="/home#myCarousel" data-slide="prev" class="carousel-control left">&lsaquo;</a><a href="/home#myCarousel" data-slide="next" class="carousel-control right">&rsaquo;  </a>
      </div>
    </div><br/>
    <div class="p lead">New Cash Donation Grantees<span class="pull-right"><a class="btn">View all Cash Donation Grantees</a></span></div>
    <hr/>
    <div align="center" class="row-fluid">
      <div id="myCarousel2" class="carousel slide">
        <div class="carousel-inner">
          <div class="item active">
            <div style="width:85%" class="container">
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More</a>
                </div>
              </div>
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More</a>
                </div>
              </div>
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More</a>
                </div>
              </div>
            </div>
          </div>
          <div class="item">
            <div style="width:85%;" class="container">
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More</a>
                </div>
              </div>
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More</a>
                </div>
              </div>
              <div class="span4">
                <div class="well profile2">
                  <div class="picture"></div>
                  <div class="name"></div>Name of Student<br/><span class="label label-success">Fully Funded!</span><br/><br/><b>Money Raised | # of Donors</b><br/>
                  <div class="progress">
                    <div style="width: 100%" class="bar bar-success"></div>
                  </div><a href="#/view-studentdetails" class="btn btn-success">Read More   </a>
                </div>
              </div>
            </div>
          </div>
        </div><a href="/home#myCarousel2" data-slide="prev" class="carousel-control left">&lsaquo;</a><a href="/home#myCarousel2" data-slide="next" class="carousel-control right">&rsaquo;</a>
      </div>
    </div>
    <div class="p lead">New Scholars<span class="pull-right"><input type="text" placeholder="Search" style="height:30px" class="search-query span2"/></span></span>
      <hr/>
    </div>
    <div align="center" class="row-fluid">
      <div class="span4">
        <div class="well profile2">
          <div class="picture"></div>
          <div class="name">Name of Student<br/><span class="label label-info">On-going</span><br/><br/><b>Money Raised | # of Donors</b><br/>
            <div class="progress">
              <div style="width: 60%" class="bar"></div>
            </div><a href="#/view-studentdetails" class="btn btn-primary">Donate</a>
          </div>
        </div>
      </div>
      <div class="span4">
        <div class="well profile2">
          <div class="picture"></div>
          <div class="name">Name of Student<br/><span class="label label-info">On-going</span><br/><br/><b>Money Raised | # of Donors</b><br/>
            <div class="progress">
              <div style="width: 10%" class="bar"></div>
            </div><a href="#/view-studentdetails" class="btn btn-primary">Donate</a>
          </div>
        </div>
      </div>
      <div class="span4">
        <div class="well profile2">
          <div class="picture"></div>
          <div class="name">Name of Student<br/><span class="label label-info">On-going</span><br/><br/><b>Money Raised | # of Donors</b><br/>
            <div class="progress">
              <div style="width: 25%" class="bar"></div>
            </div><a href="#/view-studentdetails" class="btn btn-primary">Donate</a>
          </div>
        </div>
      </div><a class="btn btn-large btn-primary">View All Students Needing Cash Donations</a><br/><br/><br/>
    </div>
</div>